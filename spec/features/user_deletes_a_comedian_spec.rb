RSpec.describe "as a visitor" do
  describe "when I click a delete comedian button" do
    ...
    it "should delete the comedian and refresh all commedians view" do

      visit '/comedians'

      within("main") do
        expect(page).to have_css("article", :count => 3)
        expect(page).to have_content("Louis C.K.")
        expect(page).to have_content("Chris Rock")
        expect(page).to have_content("Jerry Seinfeld")
      end

      find("#delete-comedian-1").click

      within("main") do
        expect(page).to have_css("article", :count => 2)
        expect(page).to_not have_content("Louis C.K.")
        expect(page).to have_content("Chris Rock")
        expect(page).to have_content("Jerry Seinfeld")
      end
